return function(entity)
    if not ai or not ai.get_blackboard then
        log_debug("WARNING: ai.get_blackboard not available for builder entity: " .. tostring(entity))
        return
    end
    local bb = ai.get_blackboard(entity)
    if not bb then
        log_debug("WARNING: Could not get blackboard for builder entity: " .. tostring(entity))
        return
    end
    ...
    log_debug(string.format("Builder %s initialized: hunger=%.0f, energy=%.0f",
