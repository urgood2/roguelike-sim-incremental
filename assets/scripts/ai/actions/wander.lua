...
-- NOTE: This action is deprecated - use idle_wander.lua instead
-- Keeping this file to avoid "action not found" errors but with very high cost
return {
    name = "wander",
    cost = 999, -- High cost to ensure idle_wander is preferred
    pre = { wander = false, _deprecated = true },  -- Impossible precondition
    post = { wander = true },

    start = function(e)
        log_debug("wander.lua (deprecated): started for", e)
    end,
    ...
    finish = function(e)
        log_debug("wander.lua (deprecated): finished for", e)
    end
}
