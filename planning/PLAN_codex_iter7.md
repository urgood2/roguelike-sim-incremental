# Incremental Game ASCII UI & Content Expansion (PLAN v6 — Implementation-Ready)
...
Any leftover pixels (`ENGINE_SCREEN_W - SCREEN_W`, `ENGINE_SCREEN_H - SCREEN_H`) are **non-interactive margin**; UI and world viewport must not rely on them.
...
- 0.2 Implement world viewport (sidebar reservation + aligned dims) — touches `sim_scene.lua`
...
### init()
- Compute `{SCREEN_W, SCREEN_H}` once from `ui_layout.aligned_screen(TILE_SIZE, globals.screenWidth(), globals.screenHeight())`.
...
4) UI click handling:
   ...
   - compute `SCREEN_W/SCREEN_H` once per frame (or reuse cached if unchanged)
