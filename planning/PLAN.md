# Incremental Game ASCII UI & Content Expansion (PLAN v7 — Implementation-Ready)
...
**Critical Path**
1) Lock layout + input contract (screen/world spaces + sidebar reservation + grid alignment)
...
### Grid-aligned screen dimensions (required)
Because `ENGINE_SCREEN_W/H` are not guaranteed to be multiples of `TILE_SIZE`, all ASCII UI layout must use:
- `SCREEN_W = math.floor(ENGINE_SCREEN_W / TILE_SIZE) * TILE_SIZE`
- `SCREEN_H = math.floor(ENGINE_SCREEN_H / TILE_SIZE) * TILE_SIZE`

Any leftover pixels:
- `MARGIN_W = ENGINE_SCREEN_W - SCREEN_W`
- `MARGIN_H = ENGINE_SCREEN_H - SCREEN_H`

…are **non-interactive margin**; UI and world viewport must not rely on them.

### UI sidebar width
- `UI_SIDEBAR_W = 10 * TILE_SIZE` (200px when TILE_SIZE=20)
...
3) All UI rectangles must be aligned to tile grid: `x,y,w,h` multiples of `TILE_SIZE`, computed using `SCREEN_W/SCREEN_H`.
...
6) Any input (click/wheel) in the non-interactive margin (`mx >= SCREEN_W` or `my >= SCREEN_H`) must be treated as **consumed** (no UI actions, no world actions).
...
- `assets/scripts/tests/test_idle_ui_layout.lua`
...
- 0.2 Implement world viewport (sidebar reservation + aligned dims + margin input gating) — touches `sim_scene.lua`
...
- 1.0 UI layout helper (+ tests) — new file, low conflict
...
**Wave 2 (UI; depends on Wave 0 + 1.0 + 1.1 + 1.2)**
...
**Change**
- Update camera zoom/offset math to reserve `UI_SIDEBAR_W = 10 * TILE_SIZE` on the right, using aligned screen dims.
- Store:
  - `sim_scene._screen_w` (aligned pixels)
  - `sim_scene._screen_h` (aligned pixels)
  - `sim_scene._world_view_w` (aligned pixels)
  - `sim_scene._ui_sidebar_w` (pixels)
  - `sim_scene._tile_size` (from config)
  - (optional but recommended) `sim_scene._engine_w`, `sim_scene._engine_h` (to detect resize and recompute)

**Authoritative math**
- `SCREEN_W = floor(globals.screenWidth()/TILE_SIZE)*TILE_SIZE`
- `SCREEN_H = floor(globals.screenHeight()/TILE_SIZE)*TILE_SIZE`
- `WORLD_VIEW_W = SCREEN_W - UI_SIDEBAR_W`
- `zoomX = WORLD_VIEW_W / config.VIRTUAL_WIDTH`
- `zoomY = SCREEN_H / config.VIRTUAL_HEIGHT`
- `zoom = math.min(zoomX, zoomY)`
- Center camera within left region:
  - `cam:SetActualOffset(WORLD_VIEW_W / 2, SCREEN_H / 2)`
...
- Clicking/wheeling in the non-interactive margin (pixels outside `SCREEN_W/SCREEN_H`) has no effect (no UI action, no world action).
...
- Resources:
  - read totals (exact function / shape; must return ints or nil→0 semantics to adopt in UI)
  ...
  - if no spend helper exists, record the minimal safe contract to implement:
    ...
    - `resources.try_spend(costs_table) -> bool` (atomic; no partial spending; does not allow totals to go negative)
- Upgrades:
...
### 1.0 UI layout helper (single source of truth)
**Files**
- add: `assets/scripts/idle_game/ui/ui_layout.lua` (headless-safe)
- add test: `assets/scripts/tests/test_idle_ui_layout.lua`
...
- `layout.toast_rects(tile_size, screen_w, screen_h, max_visible) -> { {x,y,w,h}, ... }`

**Acceptance**
- Test asserts all returned rects are multiples of `tile_size`, and sidebar width equals `10*tile_size`.
...
- Internal storage order is insertion order (oldest → newest).
- Auto-dismiss: a toast is expired and removed when `age >= duration` (boundary inclusive).
- Buffer cap: if `max_buffer` exceeded after push, drop oldest entries first (from the front).
- `get_visible()` returns up to `max_visible` **newest-first** (most recent toast is element 1) to match bottom-stacked rendering.
...
- Simulates time progression and asserts:
  ...
  - `get_visible` returns newest-first and stays stable as items expire
  - buffer cap drops oldest first
...
- Every interactive UI module must expose:
  - `get_rect(tile_size, screen_w, screen_h) -> {x,y,w,h}` (screen-space pixels, grid-aligned)
...
**Layout (fixed; grid-aligned via 1.0)**
- `x=0`, `y=0`, `w=10*TILE_SIZE`, `h=6*TILE_SIZE`

**API**
- `get_rect(tile_size, screen_w, screen_h)` (returns fixed rect above)
- `update(dt, totals, foragers_text)` (pure logic; totals are ints; nil treated as 0)
- `draw(opts)` (engine-only draw; safe no-op if engine missing)
- `hit_test(mx, my, tile_size, screen_w, screen_h) -> {consumed=true/false}` (always consumes inside rect; no actions)

**Rates (authoritative; no new global dependencies)**
- Maintain `prev_totals[key]` (ints) and `accum_dt`.
- Each update:
  - `accum_dt += max(dt, 0)`
  - when `accum_dt >= 1.0`, compute `rate[key] = (totals[key] - prev_totals[key]) / accum_dt`, then set `prev_totals[key]=totals[key]` (per-key) and reset `accum_dt=0`
...
**Panel rect (authoritative; grid-aligned via 1.0)**
- `x = SCREEN_W - UI_SIDEBAR_W`, `y = 0`, `w = UI_SIDEBAR_W`, `h = SCREEN_H`
- Inside:
...
**Renderer/API**
- `get_rect(tile_size, screen_w, screen_h)`
- `update_hover(mx,my)` (optional)
- `scroll_by_wheel(wheel_delta)` only when hovered (per Layout rules) and only when `mx < SCREEN_W` and `my < SCREEN_H` (margin gating)
...
**Layout (grid-aligned via 1.0)**
- Screen space; anchored inside sidebar with 1-tile padding:
  - `pad = TILE_SIZE`
  - `toast_w = UI_SIDEBAR_W - 2*pad`
  - `toast_h = 2*TILE_SIZE`
  - `x = SCREEN_W - UI_SIDEBAR_W + pad`
  - `y_base = SCREEN_H - pad`
- For `i=1..max_visible`: toast `i` draws with bottom alignment:
  - `y = y_base - i*toast_h`

**Ordering contract (authoritative)**
- `toast_queue.get_visible()` returns newest-first, and renderer draws `visible[1]` at the bottom, stacking older toasts upward.

**Text format (authoritative)**
- Achievement unlock toast: `ACHIEVEMENT: <Title>` (Title from `achievements.get_title(id)`; fallback to id)

**Acceptance (in-engine manual)**
- Toasts stack up to `max_visible`, newest appears at the bottom, appear/disappear without jitter, and never overlap outside sidebar.
...
- Listener maintains `save_debounce_dt` and calls `SaveManager.save()` only when:
  - `pending_save==true` and `save_debounce_dt >= 1.0` (dt-accumulated; no absolute clock dependency)
  - and `SaveManager` is present (guard for headless tests)
...
**Checklist**
- Sidebar reserved; no overlap; UI rects are grid-aligned using `SCREEN_W/SCREEN_H`
- Non-interactive margins are fully blocked for click/wheel
...
- Toasts show and auto-dismiss; newest at bottom; no duplicate handlers; text format matches spec
...
### init()
- Compute initial `{SCREEN_W, SCREEN_H}` from `ui_layout.aligned_screen(TILE_SIZE, globals.screenWidth(), globals.screenHeight())`.
...
0) (resize-safe) Recompute `ENGINE_SCREEN_W/H` and `SCREEN_W/SCREEN_H` each frame; if changed, recompute camera reservation values (Phase 0.2) before processing input.
1) world sim updates (existing)
2) spawner maintenance (existing; emits creature counts)
3) UI state updates:
   - resource panel `update(dt, totals, foragers_text)`
...
4) UI click handling:
   ...
   - margin gating: if `mx >= SCREEN_W` or `my >= SCREEN_H`, mark consumed and skip all UI/world handling for that input
...
2) ASCII UI: resource panel → upgrade panel → toasts (Screen space, high z, using `SCREEN_W/SCREEN_H`)
...
- One feature per commit (viewport, SaveManager fix, ui_layout, sprites mapping, border, toast queue, achievements, each panel, signals/listener, each creature, ground items, structures + persistence, achievements persistence).
...
- upgrades list/defs:
- upgrades list/defs: `assets/scripts/idle_game/upgrades.lua` `function upgrades.get_all()` returns `UPGRADES` table keyed by upgrade id; each def includes `name`, `description`, `base_cost`, `max_level`, `effect`.
- upgrades purchase: `function upgrades.purchase(upgrade_id, resources_module) -> boolean` (true on success; false if cannot afford or already at max level). Deducts via `resources_module.add(resource, -amount)`.
- upgrades level read: `function upgrades.get_level(upgrade_id) -> number` (0 if not purchased).
- upgrades max level read: `UPGRADES[upgrade_id].max_level` (all current upgrades set to 10).
...
- entity type naming/id field: `ai.entity_types.<basename>` (loaded by `load_directory("ai.entity_types", ai.entity_types, false)` in `assets/scripts/ai/init.lua`; no `mod.name` override).
- entity type file naming convention: `assets/scripts/ai/entity_types/<basename>.lua` returns `{ initial = { ... }, goal = { ... } }` (see `assets/scripts/ai/entity_types/forager.lua`, `demo_bot.lua`, `kobold.lua`).
