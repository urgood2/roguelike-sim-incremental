# Incremental Game ASCII UI & Content Expansion (PLAN v3 — Implementation-Ready)
...
### Layout rules
1) ASCII UI draws in **Screen space** (`layer.DrawCommandSpace.Screen` or verified equivalent).
2) Idle world draws in **World space** (`layer.DrawCommandSpace.World` or verified equivalent).
...
**Wave 1 (Shared foundation; can run in parallel after 0.1 findings)**
...
- 5.2 Structure persistence verification — mostly manual verification
...
**Achievement list (12)**
- **Resources (4)** (resource keys: `food/wood/stone/gold`; totals are integer totals):
  - `res_wood_100` (wood total >= 100)
  - `res_stone_100` (stone total >= 100)
  - `res_gold_25` (gold total >= 25)
  - `res_total_500` (**sum** of `food+wood+stone+gold` totals >= 500)
- **Upgrades (3)**
  - `upg_first_purchase` (any upgrade purchased at least once)
  - `upg_any_level_5` (any single upgrade level >= 5)
  - `upg_any_maxed` (any single upgrade at max level; “max” definition must come from upgrades module state, not hardcoded)
- **Creatures (3)**
  - `crt_total_25` (total creatures >= 25)
  - `crt_specialists_10` (miners+lumberjacks+collectors+builders >= 10)
  - `crt_builder_exists` (builder count >= 1)
- **Time (2)**
  - `time_5min` (playtime >= 300s)
  - `time_15min` (playtime >= 900s)
...
- Rows 2–5: `Food/Wood/Stone/Gold` + total + rate (rate formatted as `+X.Y/s`, one decimal, clamp `-0.0` to `0.0`)
- Row 6: `FORAGERS N/MAX` (from spawner state)
...
- `model.sorted_ids(upgrades_state) -> {id,...}` (alphabetical)
...
- On `{action="buy", id=...}`: attempt purchase via upgrades module; renderer must re-use the same `ids_sorted + scroll_row` used for rendering.
...
- Achievement unlock toast: `ACHIEVEMENT: <Title>` (Title comes from achievements definitions; if missing, fallback to id)
...
- Use `core.signal_group` (or verified equivalent from 0.1) to register/unregister handlers.
...
- Maintain minimal counters/state in listener for:
  ...
  - upgrade purchases/levels
  - creature counts snapshot
- On each relevant signal, evaluate only the achievements that depend on that signal type (no “check all 12 every tick”).
- On newly unlocked achievement:
  - enqueue toast via `toast_queue.push(text, {duration=?})`
  - return or record newly unlocked ids for persistence trigger
...
- Targets rocks; on successful harvest:
  - add stone via the same resources add path as foragers, but with multiplier:
...
- Harvest cadence: same tick rate as forager harvesting (no new timers unless required).
...
- With miners present and at least one rock on the map:
  ...
  - collector visibly picks up dropped stone items and increases stone totals further
...
- Prefer nearest item (any kind); on pickup:
  - convert to resources via standard resource add path (delta = item.amount)
...
- Placement rules:
  ...
  - within world bounds
  - sprite must be one of `ascii_sprites.STRUCTURES` values; invalid rejected

**Builder behavior (specific + deterministic)**
- Every `BUILD_INTERVAL = 20` seconds:
  - if `wood >= 50` and `stone >= 25`, spend cost (deduct via standard resource subtract path if present; otherwise negative add)
  ...
  - structure sprite chosen deterministically by cycling through `STRUCTURES` list order
